...
The response will contain the following information:
  ...
  The key for the message on the kafka topic will be the message id that was returned by the receptor-controller when the original message was submitted.  The _message\_id_ will be the message id as it is passed along from the receptor mesh network.  The _in\_response\_to_ will be the in_response_to value as it is passed along from the receptor mesh network.  The _in\_response\_to_ value can be used to match the response to the original message.
